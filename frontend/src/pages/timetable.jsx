...
export default function Timetable() {
  ...
  // Hoisted data for reuse in request and UI mapping
  const faculties = [
    {id:"F1",name:"Alice",rank:"Professor"},{id:"F2",name:"Bob",rank:"Associate Professor"},{id:"F3",name:"Charlie",rank:"Assistant Professor"},
    {id:"F4",name:"David",rank:"Professor"},{id:"F5",name:"Eve",rank:"Associate Professor"},{id:"F6",name:"Frank",rank:"Assistant Professor"}
  ];
  const classrooms = [
  ...
  const generateTimetable = async () => {
    ...
    try {
      const requestData = {
        ...
        break_periods: [4], // Lunch-only break
        faculties,
        classrooms,
        class_requirements: [
      ...
      const res = await fetch('http://localhost:3000/api/v1/timetable/generate',{
        method:'POST',
  ...
  // Lookup maps based on the hoisted arrays to show readable names/ranks
  const facultyById = (function(){
    const map = {};
    (faculties || []).forEach(f => { map[f.id] = f; });
    return map;
  })();
  const roomById = (function(){
    const map = {};
    (classrooms || []).forEach(r => { map[r.id] = r; });
    return map;
  })();
              ...
              {days.map((day,di)=>(
                <tr key={di}>
                  ...
                  {periods.map(p=>{
                    ...
                    const isBreakPeriod = [4].includes(p); // Break periods
                    return (
                      <td key={p} style={{
                        ...
                        {isBreakPeriod ? (
                          ...
                          <div>
                            ...
                            <div>
                              {(() => {
                                const fac = facultyById[cls.faculty];
                                if (!fac) return cls.faculty;
                                return `${fac.name}${fac.rank ? ` (${fac.rank})` : ''}`;
                              })()}
                            </div>
                            <div>
                              {(() => {
                                const roomId = cls.classroom || cls.classroom_id;
