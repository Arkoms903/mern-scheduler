...
app.add_middleware(
    CORSMiddleware,
    allow_origins=["http://localhost:3000", "http://localhost:5000"],  # React and Node.js origins
...
async def generate_timetable(data: TimetableRequest):
    try:
        print(f"Received request with {len(data.class_requirements)} class requirements")
        ...
        print(f"Created {len(y)} decision variables")
        ...
        for fac in data.faculties:
            # Only one class per period for each faculty
            for d in DAYS:
                for p in PERIODS:
                    fac_vars = []
        ...
        if status in [cp_model.OPTIMAL, cp_model.FEASIBLE]:
                ...
                if solver.Value(var):
                    ...
                    for p in range(sp, sp + duration):
                        scheduled.append({
                            ...
                            "day": d,
                            "period": p,
                            "start_time": data.period_times.get(str(p), data.period_times.get(p, ["", ""]))[0],
                            "end_time": data.period_times.get(str(p), data.period_times.get(p, ["", ""]))[1],
                            "faculty": req.faculty,
                            "section": req.section,
                            "subject": req.subject,
            ...
            print(f"Generated {len(scheduled)} scheduled classes")
        else:
...
if __name__ == "__main__":
    import uvicorn
    print("Starting FastAPI Timetable Server...")
